...
**가상 메모리**는 물리 메모리보다 큰 주소 공간을 프로세스에게 제공하는 메모리 관리 기법입니다.
...
**장점**:
1. **메모리 추상화**: 프로세스는 연속된 주소 공간 인식
2. **메모리 확장**: 물리 메모리보다 큰 프로그램 실행
3. **프로세스 격리**: 다른 프로세스 메모리 접근 차단
4. **공유 메모리**: 같은 물리 페이지를 여러 프로세스가 공유
5. **효율적 메모리 사용**: 실제 필요한 페이지만 로드
...
**지역성 원리(Locality of Reference)** 때문입니다.
...
**이로 인해 가능한 것**:
1. **Demand Paging**: 필요할 때만 페이지 로드
2. **부분 로딩**: 전체 프로그램의 일부만 메모리에 적재
3. **스왑핑**: 안 쓰는 페이지는 디스크로 이동

**예시**:
```
1GB 프로그램이 있어도
현재 사용 중인 50MB만 물리 메모리에 적재
나머지 950MB는 디스크에 대기
```
